...
describe("createPool", () => {
  ...
  test("getPoolData: should return undefined when the pool does not exist", async () => {
    // Given
    const { ctx, contract } = fixture<GalaChainContext, DexV3Contract>(DexV3Contract).callingUser(
      users.testUser1Id
    );
  ...
  test("getDexFeeConfigration: should return DexFeeConfig if user is authorized and MSP matches", async () => {
    // Given
    const authorities = [users.testUser1Id];
    const dexConfig = new DexFeeConfig(authorities, 0.3);

    const { ctx, contract } = fixture<GalaChainContext, DexV3Contract>(DexV3Contract)
      .callingUser(users.testUser1Id)
      .savedState(dexConfig);

    // When
    const response = await contract.GetDexFeeConfigration(ctx, new ChainCallDTO());

    // Then
    expect(response.Data).toStrictEqual(dexConfig);
  });

  it("getDexFeeConfigration: should throw UnauthorizedError if user not in authorities", async () => {
    // Given
    const authorities = ["client|test-user"];
    const dexConfig = new DexFeeConfig(authorities, 0.3);
    ...
    expect(response).toEqual(
      transactionErrorMessageContains(
        `CallingUser ${users.testUser1Id} is not authorized to create or update`
      )
    );
  });
});
